...
export function KanbanBoard({ onEditTask }) {
  ...
  const { previewOverride, setPreviewOverride, lastAppliedRef, tasksWithPreview, schedule, pendingEventRef, rafIdRef } = useKanbanPreview(tasks);

  const tasksByStatus = useMemo(() => getTasksByStatus(tasksWithPreview), [tasksWithPreview]);

  const { onDragStart: onDragStartDnD, onDragEnd: onDragEndDnD, handleDragOver: handleDragOverDnD } = useKanbanDnD({
    tasks,
    dispatch,
    previewOverride,
    setPreviewOverride,
      ...
      >
        <BoardContainer>
          <SortableContext items={columnsId}>
            {DEFAULT_COLUMNS.length ? DEFAULT_COLUMNS.map((col) => (
              <BoardColumn
                key={col.id}
                column={col}
